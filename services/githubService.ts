 ...
 * using the GitHub REST API. Returns the committed commit SHA.
 */
export async function commitAndPushBlogs(
  filePath: string,
  content: string,
  ...
  if (!token || !owner || !repo) {
    ...
    // Return a mock commit SHA for local testing and simulation mode
    return `mock_sha_${Date.now().toString(16)}`;
  }
  ...
  try {
    ...
    const commitSha = data.commit?.sha || `mock_sha_${Date.now().toString(16)}`;

    console.log(`Successfully committed and pushed ${filePath} to GitHub branch ${branch} (SHA: ${commitSha}).`);
    return commitSha;
  } catch (error) {
